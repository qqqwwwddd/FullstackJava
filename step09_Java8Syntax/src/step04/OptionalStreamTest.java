...
//		System.out.println(opt10.orElseThrow(IllegalArgumentException::new));
		...
		// step02 중개 연산 //
		// Filter : Stream 내 요소 중 조건에 맞는것을 선택

		List<String> arr2 = Arrays.asList("apple", "banana", "melon");
		arr2.stream().filter(v -> v.contains("a"));// v가가지고있는게a?

		// Map : Stream 내 요소가 특정 로직 수행 후 새로운 스트림 반환할 때 사용
		List<Integer> arr3 = Arrays.asList(1, 2, 3);
		arr3.stream().map(v -> v + 1);

		// Sorted : 정렬 / 역순 정렬
		arr3.stream().sorted(); // 123
		arr3.stream().sorted(Comparator.reverseOrder()); // 321

		// step03 최종 연산 //
		// count, min, max, sum
		// foreach : 최종적으로 모든 요소 돌면서 실행시킴 iterator 비슷
		// collect : 리스트 형태로 모아줌
